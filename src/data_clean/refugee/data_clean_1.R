...
# free_case == 0 if choice_leave_family==1
data <- data %>%
  group_by(pid) %>%
  mutate(free_case = ifelse(is.na(free_case) &choice_leave_family==1, 0, free_case)) %>%
  fill(free_case, .direction = "downup") 
...
# Factors 
data <- data %>%
  mutate(employment_year_arrival = factor(employment_year_arrival, levels = c(0,1),
                                          labels = c("no", "yes")),
